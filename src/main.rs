...
struct Args {
    ...
    /// Field delimiter
    #[arg(short, long, value_name = "OUTPUT_DELIMITER")]
...
// --------------------------------------------------
fn parse_delimiter(delimiter: String) -> Result<u8> {
...
mod unit_tests {
    use super::{
        extract_bytes, extract_chars, extract_fields, parse_delimiter,
        parse_pos,
    };
    ...
    fn test_parse_delimiter() {
        let res = parse_delimiter("".to_string());
        assert!(res.is_err());
